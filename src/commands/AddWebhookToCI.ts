...
export function addWebhookToTravis(project: Project, url: string): Promise<Project> {
    ...
    if (!file) {
        return Promise.reject(new Error(`File ${travisConfigPath} not found`));
    }
    ...
    try {
        ...
        return Promise.reject(new Error(`failed to parse Travis config ${travisConfigPath}: ${JSON.stringify(e)}`));
    }
